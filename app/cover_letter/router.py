...
async def download_cv(
    ...
    # S3: redirect to presigned URL — no bandwidth through our server
    presigned = get_presigned_url(cv.pdf_path)
    if presigned:
...
@router.post(
    ...
    summary="Generate a PDF cover letter for a matched job",
    response_class=StreamingResponse,
)
async def create_cover_letter(
    ...
    return StreamingResponse(
        io.BytesIO(pdf_bytes),
        media_type="application/pdf",
        headers={"Content-Disposition": f'inline; filename="cover_letter_{company_slug}.pdf"'},
    )
