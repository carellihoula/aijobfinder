...
import {
  ...
  Settings, LogOut, PanelLeftClose, X,
...
export default function Sidebar() {
  ...
  // Below lg the sidebar is a temporary drawer, not a rail you'd want to
  // shrink to icons - full labels regardless of the persisted `collapsed` pref.
  ...
  return (
    <aside
      className={`app-sidebar fixed top-0 left-0 bottom-0 z-40 flex flex-col overflow-hidden ${
        mobileOpen ? "translate-x-0" : "-translate-x-full"
      } lg:translate-x-0`}
      style={{
        ...({ "--sidebar-w": `${w}px` } as React.CSSProperties),
    ...
    >
      ...
      >
        {effectiveCollapsed ? (
          /* Collapsed (desktop only): logo seul, cliquable pour ouvrir */
          <button
            onClick={toggle}
          ...
          /* Expanded (desktop) or mobile drawer: logo + nom + bouton collapse/fermer */
          <>
            ...
            <button
              ...
              title={isDesktop ? "Réduire la sidebar" : "Fermer le menu"}
            ...
            >
              {isDesktop ? <PanelLeftClose className="h-3.5 w-3.5" /> : <X className="h-3.5 w-3.5" />}
            </button>
          </>
        )}
      </div>
              ...
              {section.items.map(({ to, icon: Icon, label, badge }) => {
                ...
                return (
                  <Link
                    ...
                    onClick={() => { if (collapsed) toggle(); closeMobile() }}
      ...
      >
        <Link
          to="/profile"
          title={effectiveCollapsed ? "Mon profil" : undefined}
          onClick={() => { if (collapsed) toggle(); closeMobile() }}
        ...
        >
          <UserAvatar
            ...
            size={6}
          />
          {!effectiveCollapsed && <span className="whitespace-nowrap">Mon profil</span>}
        </Link>
        <button
          onClick={logout}
        ...
        >
          ...
          {!effectiveCollapsed && <span className="whitespace-nowrap">Déconnexion</span>}
        </button>
      </div>
    </aside>
  )
